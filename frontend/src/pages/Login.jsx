...
export default function Login() {
  ...
  async function onVerifyOtp() {
    ...
    try {
      ...
      const premium = await premiumEstimate({
        city: form.city,
        zone: DEFAULT_ZONE_BY_CITY[form.city] || 'Andheri',
        platform: form.platform,
      ...
      if (premium?.success) {
        ...
        setPendingSession({ token: verified.token, worker, premium });
        setScreen('premium');
      } else {
        setWorkerSession({ token: verified.token, worker });
        setPendingSession({ token: verified.token, worker });
        setScreen('success');
      }
    } catch (err) {
